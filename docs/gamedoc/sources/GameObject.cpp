...
{	// Vérifie si l'on a bien un script
	...
	{	// 1) Vérifie les conditions et exécute les scripts 
		...
		for (int ind = SCN_TRIGGER_SCRIPT_FIRST ; ind <= SCN_TRIGGER_SCRIPT_LAST ; ind++)
		...
		// 3) Exécute aussi les scripts faisant parti du scénario
		// --> spécial pour l'insant, un objet s'occupe du scénario
		pScript->LaunchScript(SCN_TRIGGER_SCENARIO, this, NULL);
