...
class TDirectScreen
{
    public:                                     // VARIABLES
     // Variables purement DirectX
		...
		TPixelFormat *pixelFormat;	// Format des pixels de la surface principale
    ...
    public:                                     // FONCTIONS
		...
		WORD RGB24to16(DWORD col);						// Transforme RGB(8:8:8) en RGB(5:6:5) ou RGB(5:5:5) suivant la carte video =//
		WORD RGBto16(DWORD red, DWORD green,DWORD blue);// Transforme R(8)G(8)B(8) en RGB(5:6:5) ou RGB(5:5:5) suivant la carte video
	private:
		void setPixelFormat(DDSurface *surf);			// set @pixelFormat attribute of the main surfaces of the screen
};
...
class DDSurface
{
    public:                                             // FONCTIONS
		...
		int	getPixelFormat(LPDDPIXELFORMAT lpDDPixelFormat);			// Get back the pixedl format of this surface
    public:                                             // VARIABLES
...
class SpriteFrameList
{
  public:
	...
	Clear();					// Supprime toutes les SpriteFrame de la liste
