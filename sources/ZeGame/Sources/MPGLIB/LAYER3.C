...
void init_layer3(int down_sample_sblimit)
{
  ...
  for(i=-256;i<118+4;i++)
    gainpow2[i+256] = (real)pow((reallong)2.0,-0.25 * (reallong) (i+210) );

  for(i=0;i<8207;i++)
    ispow[i] = (real)pow((reallong)i,(reallong)4.0/3.0);

  for (i=0;i<8;i++)
  {
    static reallong Ci[8]={-0.6f,-0.535f,-0.33f,-0.185f,-0.095f,-0.041f,-0.0142f,-0.0037f};
    reallong sq=(reallong)sqrt(1.0+Ci[i]*Ci[i]);
    aa_cs[i] = (real)(1.0/sq);
  ...
  for(i=0;i<18;i++)
  {
    win[0][i]    = win[1][i]    = (real)(0.5 * sin( M_PI / 72.0 * (reallong) (2*(i+0) +1) ) / cos ( M_PI * (reallong) (2*(i+0) +19) / 72.0 ));
    win[0][i+18] = win[3][i+18] = (real)(0.5 * sin( M_PI / 72.0 * (reallong) (2*(i+18)+1) ) / cos ( M_PI * (reallong) (2*(i+18)+19) / 72.0 ));
  }
  for(i=0;i<6;i++)
  {
    win[1][i+18] = (real)(0.5 / cos ( M_PI * (reallong) (2*(i+18)+19) / 72.0 ));
    win[3][i+12] = (real)(0.5 / cos ( M_PI * (reallong) (2*(i+12)+19) / 72.0 ));
    win[1][i+24] = (real)(0.5 * sin( M_PI / 24.0 * (reallong) (2*i+13) ) / cos ( M_PI * (reallong) (2*(i+24)+19) / 72.0 ));
    win[1][i+30] = win[3][i] = 0.0;
    win[3][i+6 ] = (real)(0.5 * sin( M_PI / 24.0 * (reallong) (2*i+1) )  / cos ( M_PI * (reallong) (2*(i+6 )+19) / 72.0 ));
  }

  for(i=0;i<9;i++)
    COS9[i] = (real)cos( M_PI / 18.0 * (reallong) i);

  for(i=0;i<9;i++)
    tfcos36[i] = (real)(0.5 / cos ( M_PI * (reallong) (i*2+1) / 36.0 ));
  for(i=0;i<3;i++)
    tfcos12[i] = (real)(0.5 / cos ( M_PI * (reallong) (i*2+1) / 12.0 ));

  COS6_1 = (real)cos( M_PI / 6.0 * (reallong) 1);
  COS6_2 = (real)cos( M_PI / 6.0 * (reallong) 2);

  for(i=0;i<12;i++)
  {
    win[2][i]  = (real)(0.5 * sin( M_PI / 24.0 * (reallong) (2*i+1) ) / cos ( M_PI * (reallong) (2*i+7) / 24.0 ));
    for(j=0;j<6;j++)
      COS1[i][j] = (real)cos( M_PI / 24.0 * (reallong) ((2*i+7)*(2*j+1)) );
  }
  ...
  for(i=0;i<16;i++)
  {
    reallong t = (reallong)tan( (reallong) i * M_PI / 12.0 );
    ...
    for(j=0;j<2;j++) {
      reallong base = (reallong)pow(2.0,-0.25*(j+1.0));
      reallong p1=1.0,p2=1.0;
      if(i > 0) {
        if( i & 1 )
          p1 = (reallong)pow(base,(i+1.0)*0.5);
        else
          p2 = (reallong)pow(base,i*0.5);
      }
      pow1_1[j][i] = (real)p1;
      pow2_1[j][i] = (real)p2;
...
int do_layer3(struct frame *fr,unsigned char *pcm_sample,int *pcm_point)
{
  ...
  for (gr=0;gr<granules;gr++) 
  {
    ...
    for(ss=0;ss<SSLIMIT;ss++) {
      if(single >= 0) {
        synth_1to1_mono(hybridOut[0][ss],pcm_sample,pcm_point);
      }
      else {
        int p1 = *pcm_point;
        synth_1to1(hybridOut[0][ss],0,pcm_sample,&p1);
        synth_1to1(hybridOut[1][ss],1,pcm_sample,pcm_point);
      }
    }
  }
