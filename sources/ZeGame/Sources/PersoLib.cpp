...
#include "MapZone.h"			// Zones au sol
#include "PNJLib.h"				// Gestion des PNJ
#include "SoundEffect.h"		// Designe le bruitage approprie suivant la situation
#include "MemLib.h"				// Lib. d'outils de communications (semaphores...)
#include "ForceFeedbackEffectManager.h"	// Rassemble tous les effets pour joystick a retour de force
#ifdef CBUILDER
...
#define MAX_DECAL_PIXELS  9					// Nb maximal de pixels que le perso ait droit de se décaler tout seul lors des collisions
...
{   // Librairies d'animations ou de transitions non encore chargées
    ...
    {   case (HERO1) : STLPersonnage = STLHero; break;
		...
		case (NAIN1) : STLPersonnage = STLModelePersos[CharacterType]; break;				
...
Personnage::Personnage(int PersoType):
	PhysicObject(PHYSICOBJECT_ORGANIC),
    ...
    ModelePersoType(PersoType)     // Type de personnage : HERO, OGRE, NAIN	
{
	...
	peOldUserFutSpeed = peUserFutSpeed = 100;
    peAccelerate = true;
    peSpeedTrans = false;
    peAllCollisions = true;     // Toutes les collisions sont activées
	peHasJustCollide = false;
    peNumSprite = 0;
	peSprNumLib = 0;
	...
	uLastActionUse = 0;
	uTimeIntActionUse = 1200;	// 1.2 s
	uLastActionHit = 0;
	uTimeIntActionHit = 800;	// 0.8 s
	...
	uLastCollisionTime = 0;		
	uTimeIntCollision = 5000;	// 5 sec
}
//----------------------------------------------------------------------------//
...
void Personnage::Keyb2FutEtat(void)
{
    ...
    if ((Key_K) && (peFutEtat != MEURT1) && (peEtat != MEURT1))
    {	if ((peFutEtat != MORT1) && (peEtat != MORT1))
		{	peFutEtat = MEURT1; peFutDirection = peDirection; 
		} else
		if ((peFutEtat != RESSUSCITE1) && (peEtat != RESSUSCITE1))
		{	peFutEtat = RESSUSCITE1; peFutDirection = peDirection; 
		}
	}
	else
    if (Key_SPACE)	// Se prend un coup ds la tetê
     { peFutEtat = PRENDCOUP1; peFutDirection= peDirection;
       ...
       {  // SANS L'UTILISATION DES SCRIPTS
		  ...
		  unsigned int currentTime = GameHistory->GetTime();
		  if ((currentTime - uLastActionUse) > uTimeIntActionUse)
		  {	  
 			  uLastActionUse = currentTime;
    ...
    if (Key_GRAVE == EVENTKEY_APPUYE)  // Change d'arme
    {  ArmeEnMain++;
       if (ArmeEnMain >= NbWeapons) ArmeEnMain = 0;
        ...
        //  => on s'arrête
    if (    (!Key_K) && (!Key_SPACE) && (!Key_LCTRL) && (!Key_GRAVE) 
         ...
         && (peEtat!=ARRET1) && (peEtat!=STAND1) && (peEtat!=MORT1) && (peEtat!=SAUTE1)
         && (peEtat!=DEMITOUR1) && (peEtat!=RESSUSCITE1)
		 && (peFutEtat!=MEURT1))
	...
	if ( (Key_BACKSLASH) &&  
	 	 ( (peEtat==ARRET1) || (peEtat==MARCHE1) || (peEtat==COURSE1) || (peEtat==STAND1))
		 )
	{	peFutEtat = SAUTE1;
	} 
	else
...
/*        if ((peEtat==MARCHE1)
	...
	if (Key_LALT)
	{	if (DI->isAnalogic())
		{	if (peUserFutSpeed!=DI->getAnalogicPercent())
			{	peOldUserFutSpeed = peUserFutSpeed;
				peUserFutSpeed = DI->getAnalogicPercent();
			}
		} else
		{	if (peUserFutSpeed!=40)
			{	peOldUserFutSpeed = peUserFutSpeed;
				peUserFutSpeed = 40;
			}
		}
...
void Personnage::peBouge()
{
	...
	{	peNumSprite=0;
		peNumSpriteInterm = 0;
		// Mecanisme de synchronisation sur fin d'anim
...
void Personnage::ResultAction(void)
{
	...
	if (    ((peEtat == DONNECOUP1) || (peEtat == COUPEPEE1))
		 && (peNumSprite == 10) )	// [ACF]
	{
		unsigned int lastActionHit = uLastActionHit;
		// a) Si il y'a un objet devant lui
		if (FrontObject.decors != NULL)
		{
			  ...
			  if ((curentTime - lastActionHit) > uTimeIntActionHit)
			  {	  // Envoi un PLAYER_FRAPPER event pour prevenir le decors que le 
				  // perso vient de le frapper
		...
		// b) Si y'a un enemi devant
		Personnage *perso = GetPersoTouche();
		if ((perso != NULL) && (perso->isAlive()))
		{	  unsigned int curentTime = GameHistory->GetTime();
			  if ((curentTime - lastActionHit) > uTimeIntActionHit)
			  {	  // Envoi un PLAYER_FRAPPER event pour prevenir le personnage
				  // qu'on vient de le frapper
 				  uLastActionHit = curentTime;
				  CreateEvent( EVENTS_STATUS_DIRECT, EVENTS_TYPE_JOUEUR, 
 							   SCN_TRIGGER_PLAYER_FRAPPER, perso);
				  
				  // Calcul des degats causes et deduction des points de vie
				  perso->hitBy(this);

				  // Joue le bruitage approprie suivant l'arme qu'il a en main,
				  // le personnage d'en face, l'arme que le perso a, ou il l'a touche ...
				  GameSoundEffectManager->playFightSoundAttack(this, perso);
			  }
		}
	} else
	if ((peEtat == TIRER1) && (peNumSprite == 8))
	{	GameSoundEffectManager->playShootSound(this);
	}
}
//----------------------------------------------------------------------------//
...
{   // Vérifie si le perso ne vient pas tout juste de changer d'anim
    ...
    {  peAnimSpeed = 1.0;	// par defaut
		
	   // a) Transition MARCHE->COURSE
       if ((peEtat==COURSE1) && (peOldEtat==MARCHE1))
       {  // Transition sur 2 pas
          peFutSpeed = WorldModel.PAS_HOMME_COURSE;
		  if (peUserFutSpeed!=100)
		  {	peFutSpeed = peFutSpeed*peUserFutSpeed/100;
			peAnimSpeed = computeAnimSpeed(peUserFutSpeed);
		  }
       ...
       else // i) Transition DEMITOUR1->MARCHE
	   ...
	   else // j) Transition MARCHE->SAUT
	   if ((peEtat==SAUTE1) && (peOldEtat==MARCHE1))
	   {  // Sursaut d'accélération à l'amorce du saut
          peSpeed += peSpeed/2;
		  // Puis légère déccélération au fur et à mesure
		  // Transition sur 4 pas 
		  peFutSpeed = 0;
		  pedVit = (peFutSpeed - peSpeed) / (peNbSprite*2); 	
		  peOldSpeed = peSpeed;
		  peAccelerate = false;
		  peSpeedTrans = true;
	   } 
	   else // k) Transition SAUT->MARCHE
	   if ((peEtat==MARCHE1) && (peOldEtat==SAUTE1))
	   {  // Reprise de l'allure normale de marche
		  // Transition sur 2 pas 
		  peFutSpeed = WorldModel.PAS_HOMME_MARCHE;
		  pedVit = (peFutSpeed - peSpeed) / peNbSprite; 	
		  peOldSpeed = peSpeed;
		  peAccelerate = (peFutSpeed > peSpeed);
		  peSpeedTrans = true;
	   }
   	   else // l) Transition SAUT-> ARRET,DONNECOUP 
	   if ( (peOldEtat==SAUTE1) 
			&& ((peEtat==ARRET1) || (peEtat == DONNECOUP1))    )
	   {  // On s'arrête en douceur en glissant sur 1 pas
		  peFutSpeed = 0;
		  pedVit = (peFutSpeed - peSpeed) * 2 / peNbSprite; 	
		  peOldSpeed = peSpeed;
		  peAccelerate = false;
		  peSpeedTrans = true;
	   }
	   else // m) Transition COURSE->SAUT
	   if ((peEtat==SAUTE1) && (peOldEtat==COURSE1))
	   {  // Sursaut d'accélération à l'amorce du saut
          peSpeed += peSpeed/2;
		  // Puis légère déccélération au fur et à mesure
		  // Transition en douceur sur 5 pas 
		  peFutSpeed = 0;
		  pedVit = (peFutSpeed - peSpeed) * 2 / (peNbSprite*5); 	
		  peOldSpeed = peSpeed;
		  peAccelerate = false;
		  peSpeedTrans = true;
	   }
	   else // n) Transition SAUT->COURSE
	   if ((peEtat==COURSE1) && (peOldEtat==SAUTE1))
	   {  // Recommance à courrir a allure normale
		  // Transition sur 2 pas 
		  peFutSpeed = WorldModel.PAS_HOMME_COURSE;
		  pedVit = (peFutSpeed - peSpeed) / peNbSprite; 	
		  peOldSpeed = peSpeed;
	   ...
	   else // o) Transition ARRET1, DONNECOUP, STAND ... ->SAUT
	   if ( (peEtat==SAUTE1) &&
		    ((peOldEtat==ARRET1) || (peOldEtat==STAND1) || (peOldEtat==DONNECOUP1) || (peOldEtat==COUPEPEE1))
		  )
	   {  // Sursaut d'accélération à l'amorce du saut
          peSpeed += 2*WorldModel.PAS_HOMME_MARCHE/3;
		  // Puis légère déccélération au fur et à mesure
		  // Transition sur 4 sauts 
		  peFutSpeed = 0;
		  pedVit = (peFutSpeed - peSpeed) / (peNbSprite*2); 	
		  peOldSpeed = peSpeed;
		  peAccelerate = false;
	...
	// Si aucune animation n'a changee, c'est peut-etre seulement la vitesse
	// qui a changee
	if (peOldUserFutSpeed!=peUserFutSpeed)
	{	
	   // Perso en train de marcher ou de courir
	   if ((peEtat==MARCHE1) || (peEtat==COURSE1))
       {  // Transition en 2 pas
		  if      (peEtat==MARCHE1) peFutSpeed = WorldModel.PAS_HOMME_MARCHE;
		  else if (peEtat==COURSE1) peFutSpeed = WorldModel.PAS_HOMME_COURSE;
		  peFutSpeed = peFutSpeed*peUserFutSpeed/100;
          pedVit = (peFutSpeed - peSpeed) / peNbSprite;
          peOldSpeed = peSpeed;
          peAccelerate = peSpeed>peOldSpeed;
          peSpeedTrans = true;
		  peAnimSpeed = computeAnimSpeed(peUserFutSpeed);
       }
...
// userFutSpeed en %
float Personnage::computeAnimSpeed(int userFutSpeed)
{	if (userFutSpeed<50)
	{	if (userFutSpeed<20) return 0.25;
		return 0.5;
	}
	return 1.0;
}
...
{   // Coordonnées sur lesquelles on va travailler
    ...
    if (   (peEtat == MARCHE1) || (peEtat == COURSE1) || (peEtat == SAUTE1)
...
/*    if (COLLIDE_MODE==COLLIDE_PIXEL)
    ...
    if (peAllCollisions)
    {
      ...
      { // 0) Au départ, on suppose qu'il n'y a pas de collisions
        ...
        {	// Si l'on a souhaité marcher, sauter ou courir -> pas la peine d'aller + loin
			if ((peEtat==MARCHE1) || (peEtat==COURSE1) || (peEtat==SAUTE1))
        	{	Collision = true;
	        }
        }
        ...
        {  Collision = DetectColis_DecorsCircle(newPersoNivX, newPersoNivY, newPersoNivU, newPersoNivV, newPeX, newPeY, newPeU, newPeV);
        }

         // 1) Regarde tout d'abord si les pieds du persos ne sont pas sur un décors infranchissable
        ...
        // 3.1) Si l'on n'a pas de collisions a ce niveau là, on pousse le test en essayant
		//	  de detecter les collisions avec des objets du decors
        if (!Collision)
        { // *** TEST COLLISIONS ***
           
		  // [AVIRER] Juste en attendant que les collisions ne soient pas faites
		  if ((peEtat!=COURSE1) && (peEtat!=SAUTE1))
		...
		// 3.2) Detection des collisions avecd d'autres personnages
		if (!Collision)
		{	Collision = DetectColis_Personnages(newPersoNivX, newPersoNivY, newPersoNivU, newPersoNivV, newPeX, newPeY, newPeU, newPeV);	
		}
        ...
        {  // On NE change PAS les coordonnées du perso : on ne tient pas compte des nouvelles
		   ...
		   // Declenche : effets a retour de force, bruitages, ...
		   collisionDetected();
        }
        else
    ...
    { ViewMap->OldScreenX = ViewMap->ScreenX;
	  ViewMap->OldScreenY = ViewMap->ScreenY;
	  ViewMap->ScreenX = peX - ViewMap->ScreenViewX/2;
      ViewMap->ScreenY = peY - ViewMap->ScreenViewY/2;
      ViewMap->PosCoinUV();
...
{    const int BlocPerimetre = 4;          // S'intéresse à une zone rectangulaire formée de blocs autour du personnage
     ...
     { for (int v = -BlocPerimetre ; v <= +BlocPerimetre ; v++)
       { // Position d'un bloc autour du perso
         ...
         if ((posU>= 0) && (posV>=0) && (posU<MapDecors->MapWidth) && (posV<MapDecors->MapHeight))
...
bool Personnage::DetectColis_Personnages(double &newPersoNivX, double &newPersoNivY, double &newPersoNivU, double &newPersoNivV, int &newPeX, int &newPeY, int &newPeU, int &newPeV)
{	// [PROVISOIRE] Rayon a determiner d'apres les caracteristiques des 2 persos
	static double rayon = 1.0;
	// Pour l'instant, on detecte seulement si il y'a un ou plusieurs personnages dans
	// un certains perimetre autour des ce personnage
	for (unsigned int i=0; i < LGP.getCharacterCount(); i++)
	{	Personnage *perso = LGP.getPersoAtIndex(i);
		if ((perso!=NULL) &&(perso->GetID() != GetID()))
		{	// On a trouve un personnage dans ce cercle qui est potentiellement 
			// en collision avec ce personnage
			if (perso->isInsideCircle(newPersoNivX, newPersoNivY, rayon))
			{	bool collision = false;
				// Si ils sont tous les 2 a l'arret -> pas de collision
				// -> ca permet au personnage de quand-meme tourner sur lui-meme
				collision = !((peSpeed==0) && (perso->peSpeed==0));
				if (collision) return true;

				// Si ils se dirigent dans des direction opposees
				collision = isMovingTowards(perso);
				if (collision) return true;
			}
		}
	}
...
bool isCloseDirection(eMapDirection dir1, eMapDirection dir2)
{	if (dir1 == dir2) return true;
	switch (dir1)
	{	case Map_N : if ((dir2==Map_NE) || (dir2==Map_NO)) return true;
		case Map_S : if ((dir2==Map_SE) || (dir2==Map_SO)) return true;
		case Map_E : if ((dir2==Map_NE) || (dir2==Map_SE)) return true;
		case Map_O : if ((dir2==Map_NO) || (dir2==Map_SO)) return true;
		case Map_NE: if ((dir2==Map_N ) || (dir2==Map_E )) return true;
		case Map_NO: if ((dir2==Map_N ) || (dir2==Map_O )) return true;
		case Map_SE: if ((dir2==Map_S ) || (dir2==Map_E)) return true;
		case Map_SO: if ((dir2==Map_S ) || (dir2==Map_O)) return true;
	}
	return false;
}

// === Renvoi vrai si l'un des 2 persos (au moins) se dirige dans la  ========//
// direction de l'autre
bool Personnage::isMovingTowards(Personnage *perso)
{	
	// Attention : getDirection() utilise les coordonnees actuelles des persos
	// et non les futures. Ca devrait marcher parcequ'ils ne se teletransportent
	// pas et que les deplacements sont de petits ordres de grandeur
	eMapDirection dir = getDirection(perso);
	if ((peSpeed!=0) && (isCloseDirection(dir, (eMapDirection) peFutDirection))) return true;
	
	dir = perso->getDirection(this);
	if ((perso->peSpeed!=0) && (isCloseDirection(dir, (eMapDirection) perso->peFutDirection))) return true;
	
	return false;
}
//----------------------------------------------------------------------------//

// === Renvoi la direction cardinale la plus coherente entre les positions ===//
// de 2 personnages : de ce personnage -> vers le personnage @dest
eMapDirection Personnage::getDirection(Personnage* dest)
{	double xB = dest->PersoNivX - PersoNivX;
	double yB = dest->PersoNivY - PersoNivY;
	
	if ((xB>0) && (yB<=-2*xB)) return Map_S;
	if ((xB<0) && (yB<= 2*xB)) return Map_S;

	if ((xB<0) && (yB>=-2*xB)) return Map_N;
	if ((xB>0) && (yB>= 2*xB)) return Map_N;

	if (xB>0)
	{	if ((yB>0) && (yB<= xB/2)) return Map_E;
		if ((yB<0) && (yB>=-xB/2)) return Map_E;
	} else
	{	if ((yB>0) && (yB<=-xB/2)) return Map_O;
		if ((yB<0) && (yB>= xB/2)) return Map_O;
	}

	if ((xB>0) && (yB>0)) return Map_NE;
	if ((xB>0) && (yB<0)) return Map_SE;
	if ((xB<0) && (yB>0)) return Map_NO;
	if ((xB<0) && (yB<0)) return Map_SO;
...
void Personnage::SetCoords(double X, double Y)
{	PersoNivX = X;
    PersoNivY = Y;
    PersoNivXY2UV();
    PersoNivXYtoPeXY();
...
//=== Indique si ce personnage est à l'intérieur de la zone n°zoneID =========//
bool Personnage::isCharacterInZone(MapZone *zone)
{	return (zone->isPointInZone(PersoNivX, PersoNivY));
}
//----------------------------------------------------------------------------//
...
//=== Renvoi vrai si le perso est dans la zone n°'id' ========================//
bool Personnage::isInInsideZone(int id)
...
//=== Efface telle zone ======================================================//
void Personnage::DelInsideZone(int id)
{	lInsideZone.remove(id);	
}
//=== Ajoute une zone ========================================================//
void Personnage::AddInsideZone(int id)
{	lInsideZone.push_back(id);
}
//----------------------------------------------------------------------------//


//=== Renvoi un ptr vers le personnage que l'on vient de toucher =============//
Personnage* Personnage::GetPersoTouche()
{	// Vérifie si y'a pas un perso juste en face de soi
	// -> regarde si devant soi, dans un cercle de rayon 1 mètre, il y'a pas un perso
	// La portee depend aussi de l'arme dont il s'est servi (lance vs poignard)
	
	double portee = 1;		// Portee et rayon par defaut	
	double rayon = 0.7;
	...
	double portee_HV   = portee;
    double portee_Diag = portee / sqrt(2);

	// Convertion Direction Ecran -> Direction Carte
	eMapDirection peDirectionMap = (eMapDirection) peDirection;
	
	// Direction à l'écran
	double circleX = PersoNivX; 
	double circleY = PersoNivY;
	switch (peDirectionMap)   
	{   // Deplacement en diagonal sur la Carte
		case Map_SO : circleX-=portee_Diag; circleY+=portee_Diag; break;
		case Map_SE : circleX+=portee_Diag; circleY+=portee_Diag; break;
		case Map_NE : circleX+=portee_Diag; circleY-=portee_Diag; break;
		case Map_NO : circleX-=portee_Diag; circleY-=portee_Diag; break;
		// Déplacements horizontaux
		case Map_E : circleX+=portee_HV; break;
		case Map_O : circleX-=portee_HV; break;
		// Déplacements verticaux
		case Map_S : circleY+=portee_HV; break;
		case Map_N : circleY-=portee_HV; break;
	};

	return LGP.GetFirstPersoInCircle(circleX, circleY, rayon, GetID());
}
//----------------------------------------------------------------------------//

//=== Indique si le personnage se trouve dans ce cercle ou pas ===============//
bool Personnage::isInsideCircle(double& centX, double &centY, double rayon)
{	return ( (  (PersoNivX - centX)*(PersoNivX - centX) 
			   +(PersoNivY - centY)*(PersoNivY - centY)) 
			< rayon*rayon); 
}
//----------------------------------------------------------------------------//

//=== Comportement du joueur en cas de collisions ============================//
// Declenchement des effets a retour de force, bruitages, ...
void Personnage::collisionDetected(void)
{	// Effet a retour de force si :
	// - c'est le joueur que le perso dirige,
	// - le perso etait en train de courir
	// - on n'en a pas joue un depuis 5 sec
	if (DI->isForceFeedback())
	{	if ((ModelePersoType==HERO1) && (peOldEtat==COURSE1))
		{	
			unsigned int curentTime = GameHistory->GetTime();
			if ((curentTime - uLastCollisionTime) > uTimeIntCollision)
			{	uLastCollisionTime = curentTime;
				g_ForceFeedbackEffectManager.playBumpIntoWall(this, NULL);
			}
		}
	}
}
...
//=== Calcul des degats causes et deduction des points de vie ================//
void Personnage::hitBy(Personnage* opponent)
{	// Cas provisoire pour tous les Ogres
	if (ModelePersoType == OGRE1)
	{	iLife -=  (rand() % 50);
		if (iLife<=0) bDead = true;	
		
		if (bDead)
		{	CreateEvent(EVENTS_STATUS_DIRECT, EVENTS_TYPE_JOUEUR, SCN_TRIGGER_PERSO_DEAD, this);
		}
	}
}
//----------------------------------------------------------------------------//
