...
#define SCRIPT_ERROR_NOTANOPTION	4		// Le mot-clé trouvé n'est pas une option d'une ligne de script

// ------------------------------------------------------------------------ Enum	
enum
{	SCRIPTLINE_FLAG_NO_REENTRANCE = 1		// Ne permet pas a un objet d'executer plusieurs fois en meme temps la meme ligne de script
};

enum
{	SCRIPT_FLAG_NO_REENTRANCE = 1			// Ne permet pas a un objet d'executer plusieurs fois en meme temps le meme script
};
...
{	//--------------------------------------------------- Attributs de la classe
	...
	std::vector<TAction*>	vActions;	// Liste d'actions résultants d'une ligne d'un script
	std::vector<TCause*>	vCauses;	// Conditions validant cette portion de script
	int iFlags;							// Options diverses (reentrances ...)
  ...
  public:
	...
	static bool isInvalidChar(char car);// Renvoie vrai si c'est un espace, une tab, un car. de fin de ligne ...
	...
	// Fonctions manipulants les flags
	inline bool isReentrance();			// Indique si une meme ligne de script peut etre executee plusieurs fois en meme temps par un meme objet
	int setOptions(char* options);		// Parse les options

  private:
	int getParameters(char *str, char parameter[][MAX_LENGTH_KEYWORD], int &curseur);	// Get parameters of a function
	
	bool isRandomParameter(char* param);				// Indique si c'est un nombre aleatoire
	RandomType*	getRandomType(char* param);				// Renvoi le nombre aleatoire construit a partir de nombres entiers
	RandomFloatType* getRandomFloatType(char* param);	//	 "			"		"		 "			"			"   reels

	inline void setFlag(int flag);						// Rajoute une option
};
//////////////////////////// Fin de SCRIPTLINE /////////////////////////////////
...
{	// -------------------------------------------------- Attributs de la classe
  public:
	  ...
	  int *iActionMutex;		// Tableau indexant les numero d'actions possedant un mutex
	  int iActionMutexIndex;	// Indice de la prochaine case aremplir dans le tableau precedent
...
{ //--------------------------------------------------- Attributs de la classe	
  ...
  public:
	...
	bool isScriptLineRunning(int line);		// Indique si cette ScriptLine est en cours d'execution
	
	inline void SetSrcGO(GameObject* go) { pSrcGO = go; }
	inline GameObject* GetSrcGO() { return pSrcGO; }
	inline bool isScriptRunning() { return vRunningScripts.size()!=0; }		// Indique si des scripts sont en cours d'execution
};
...
{	//--------------------------------------------------- Attributs de la classe
  ...
  public:
    ...
    inline std::IDMap*  GetScriptMap();			// Récupère le dico des ID des scripts du jeu
