...
//=== Renvoie le n° de l'options de la ligne d'un script ===
int GetScriptLineOption(char* option)
{	for (int i=FIRST_SCRIPTLINE_OPTION ; i < MAX_SCRIPTLINE_OPTION ; i++)
	{	for (int j=0 ; j < NB_SCRIPT_LANGUAGE ; j++)
		{	if (strcmp(sScripLineOptions[i][j], option) == 0) 
			{	return i;
			}
		}
	}
...
//=== Renvoie le n° de l'option  d'un script ===
int GetScriptOption(char* option)
{	for (int i=FIRST_SCRIPT_OPTION ; i < MAX_SCRIPT_OPTION ; i++)
	{	for (int j=0 ; j < NB_SCRIPT_LANGUAGE ; j++)
		{	if (strcmp(sScripOptions[i][j], option) == 0) 
			{	return i;
			}
		}
	}
