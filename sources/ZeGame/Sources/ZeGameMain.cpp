...
#include "ScenarioManager.h"	// Gere les GameObjects lies au Scenario proprement dit du jeu
#include "fBeginDlg.h"			// Boîte de dialogue du démarrage	
#include "PhysicObjectManager.h"// Manage les differents objets du jeu et leurs proprietes physiques
#include "ForceFeedbackEffectManager.h"	// Rassemble tous les effets pour joystick a retour de force
#include "MPSession.h"			// Couche multi-joueur (serveur+joueur) de ZeGame
#include "MPMonitor.h"			// Affichage des informations liees au jeu multi-joueurs
#pragma hdrstop
...
{	
	...
	InitGame_PhysicObjectManager(); // Initialise le manager du Scenarios
	InitGame_GameScriptList();		// Chargement de la liste de scripts du jeu 

	InitPack_SoundManager();		// Initialise le gestionnaire d'effets sonores
	InitPack_SoundEffectManager();	// Repartition des bruitages

	InitGame_ForceFeedbackEffectManager();	// Initialise le manager des effets du joystick a retour de force

	InitGame_MPSession();			// Intialise le module massivement multi-joueur
	InitGame_MPMonitor();			// Initialise le Moniteur multi-joueurs
...
{	
	...
	//--- Initialise la gestion du Scenario du jeu ---
	InitGame_ScenarioManager();
...
{	
	...
	CloseGame_MPMonitor();			// Referme proprement le monitor multi-joueurs
	CloseGame_MPSession();			// Ferme le module massivement multi-joueur

	
	CloseGame_ForceFeedbackEffectManager();	// Referme proprement le manager

	//--- Ferme le gestionnaire d'effets sonores  ---
	DisablePack_SoundEffectManager();
	DisablePack_SoundManager();	
	...
	CloseGame_PhysicObjectManager();	// Referme proprement le manager du Scenarios
...
{	
	...
	//--- Libere le gestionnaire du Scenario du jeu ---
	CloseGame_ScenarioManager();
...
{	
	...
	DSMuzikLib->MuzikManage();				// Musiques
	GameSoundManager->SoundEffectManage();	// Bruitages
	...
	// MaJ du Scenario du Jeu
	g_ScenarioManager.update();
	...
	// Affiche des infos textutelles liees au jeu multi-joueur
	g_pMPMonitor->Update();
	...
	// Test
/*	static bool essai1 = false;
	static bool essai2 = false;	
	static bool essai3 = false;
	if ((rand()%50 == 0) && (!essai1))
	{	GameSoundManager->playSound(0);
		//essai1 = true;
	}
	if ((rand()%50 == 0) && (!essai2))
	{	GameSoundManager->playSound(1);
		//essai2 = true;
	}
	if ((rand()%50 == 0) && (!essai3))
	{	GameSoundManager->playSound(2);
		//essai3 = true;
	}*/
...
void SpecialKeys()
{
	...
	if (Key_ENTER==1)
       {     // transmutation
		#ifdef DEBUG // Chgt de perso qui plante totalement
		if (!bPerso) { Hero->peX = Ogre->peX; Hero->peY = Ogre->peY; }
        else if (Ogre!=NULL)  { Ogre->peX = Hero->peX; Ogre->peY = Hero->peY; };
    	bPerso=!bPerso;
...
{	if (DSMuzikLib != NULL)	DSMuzikLib->SetPause(bGamePause);
	if (GameSoundManager!=NULL) GameSoundManager->setPause(bGamePause);
...
{	char options[512];
	...
	// 4) résolution du jeu (résolution du jeu par défaut 640x480x16)
	ini.GetStringProfile("Options", "Resolution", options);
	if (strcmp(options, "640x480x16") == 0 )		// 640x480x16
	{	CFG_SCREEN_WIDTH  =  640; CFG_SCREEN_HEIGHT =  480; CFG_SCREEN_BITS = 16; 
	} else
	if (strcmp(options, "800x600x16") == 0 )		// 800x600x16
	{	CFG_SCREEN_WIDTH  =  800; CFG_SCREEN_HEIGHT =  600; CFG_SCREEN_BITS = 16; 
	} else
	if (strcmp(options, "1024x768x16") == 0 )		// 1024x768x16
	{	CFG_SCREEN_WIDTH  = 1024; CFG_SCREEN_HEIGHT =  768; CFG_SCREEN_BITS = 16;
	} else 
	if (strcmp(options, "1280x1024x16") == 0 )		// 1280x1024x16
	{	CFG_SCREEN_WIDTH  = 1280; CFG_SCREEN_HEIGHT = 1024; CFG_SCREEN_BITS = 16;
	} else 
	if (strcmp(options, "1600x1200x16") == 0 )		// 1600x1200x16
	{	CFG_SCREEN_WIDTH  = 1600; CFG_SCREEN_HEIGHT = 1280; CFG_SCREEN_BITS = 16;
	} else
	{	// Resolution par defaut					// 640x480x16
		CFG_SCREEN_WIDTH  =  640; CFG_SCREEN_HEIGHT =  480; CFG_SCREEN_BITS = 16;
	}
	
	// 5) Regarde si il faut afficher la boite de dialogue de setup ou directement lancer le jeu
	ini.GetStringProfile("Options", "Setup", options);
	fBeginDlg::setShow(strcmp(options, "Yes") == 0);
	
	// 6) Niveau par défaut a charger au demarrage
	options[0] = '\0';
