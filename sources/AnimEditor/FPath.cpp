...
{  // ??? dirs.data ???
   ...
   for (Container::iterator i = contVisite.begin() ; i!= contVisite.end(); ++i)
...
{  //-- Variables locales à l'algo --
   ...
   {  get_FirstNode(contOpen, N);                 // Récupère le 'meilleur' Node des OPEN
      ...
      { // Récupère une directions
        ...
        int cout;
        switch (heuristique.TypeDirection)
        {  case (TYPE_DIRECTION_GRILLE) : cout = heuristique.Cout(map,N.cN,d1,gc); break;
           case (TYPE_DIRECTION_MAP)    : cout = heuristique.Cout(map,N.cN,d2,gc);  break;
        }
