...
//#define INITGUID        // Pour pouvoir toucher aux IID_IDirectDrawSurface3
#include "DDraw.h"        // Bibliohotèque Microsoft DirectDraw
#include <stdio>
#include <vector.h>       // tableau dynamique provenant de la librairie STL
...
class DDSurface
{
    public:                                             // FONCTIONS
        ...
        DDSurface(LPDIRECTDRAW4 *pDD4,int Width,int Height,SURFACEMEM surfMem = SYSTEM_MEMORY); // Constructeur
        DDSurface(void);
...
class SpriteFrame : public DDSurface
{
  ...
  public:
    // Constructeur
    SpriteFrame::SpriteFrame(LPDIRECTDRAW4 *pDD4, int Width,int Height,SURFACEMEM surfMem = SYSTEM_MEMORY);
    SpriteFrame::SpriteFrame(TDirectScreen *pScreen, int Width,int Height,SURFACEMEM surfMem = SYSTEM_MEMORY);
    ...
    void CreateFrame(LPDIRECTDRAW4 *pDD4, void* STLperso, const char* BitmapName,const char* SpiName,int NbFrame,SpriteFrame *New_Frame, int i_IDFrame = 0,bool BmpToMem=true);
...
{
  public:
    SpriteTileLib(void);      // Constructeur
...
// SpriteTileLib *STL;  // Liste officielle des sprites;
...
{
  public:
    SpriteFrameList(void);      // Constructeur
    ...
    // 2nde version pour l'éditeur (05/12/98)
    SpriteFrame*  AddFrame(LPDIRECTDRAW4	*pDD4,void* STLperso,const char* BitmapName,const char* SpiName, int i_IDFrame=0,bool BmpToMem=true, bool texture = false); // Ajoute une frame à la liste
...
{      // 4 points
       ...
       bool Enabled;
       // Constructeur
       ZoneZoneOrientRectRect() { x1 = y1 = x2 = y2 = x3 = y3 = x4 = y4 = -1; Enabled = false;};
       // Permet de remplir les points du rectangle rapidement
