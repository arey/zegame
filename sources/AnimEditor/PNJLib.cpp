...
{  // Si le perso est inactif, on essaye de le faire aller quelque part
   ...
   if (PNJAction == PASSIF)
    if (random(100)==0)  // Une chance sur 100 de se déplacer
    { // On récupère un point au hazard sur la map
      int ptW = random(MapNiveau->PrincEtage->EtageWidth);
      int ptH = random(MapNiveau->PrincEtage->EtageHeight);
/*    if (Aller) { ptW = pt1u; ptH = pt1v; }
      else       { ptW = pt2u; ptH = pt2v; }
      Aller=!Aller;*/
      if (LetsGo(ptW,ptH))
...
/*        GridCoord gDep = CheminTrouve.back();
        PersoNivX = ((double)gDep.x+0.5)*WorldModel.SQR_LENGTH;
        PersoNivY = ((double)gDep.y+0.5)*WorldModel.SQR_LENGTH;
        PersoNivXY2UV();
        PersoNivXYtoPeXY();*/
...
{ switch (mp)
  { case (M_GAUCHE) : return or_NO; break;
    case (M_HAUT)   : return or_NE; break;
    case (M_DROITE) : return or_SE; break;
    case (M_BAS)    : return or_SO; break;
  }
  return -1;
}
//----------------------------------------------------------------------------//
...
{ for (int i=0; i < ListePersos.size(); i++)
  {  ListePersos[i]->Fut2PresEtat();
     ListePersos[i]->Update();  // <=> Keyb2FutEtat()
     ListePersos[i]->peBouge();
  }
