...
{ vSpriteVertices[0].sx = ScrX1; vSpriteVertices[0].sy = ScrY1;
  vSpriteVertices[1].sx = ScrX2; vSpriteVertices[1].sy = ScrY1;
  vSpriteVertices[2].sx = ScrX1; vSpriteVertices[2].sy = ScrY2;
  vSpriteVertices[3].sx = ScrX2; vSpriteVertices[3].sy = ScrY2;
}
//----------------------------------------------------------------------------//
...
{ HRESULT hr;
  ...
  DWORD oldZState;
  pDevice->GetRenderState(D3DRENDERSTATE_ZFUNC, &oldZState);
...
// Set any appropiate state
    ...
    pDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, D3DFVF_TLVERTEX, vSpriteVertices , 4, D3DDP_WAIT);

  // Restaure les paramètres de rendu 3D sauvegardés
  pDevice->SetRenderState(D3DRENDERSTATE_ZFUNC, oldZState);
}

void Sprite3DTile::SpriteRender(int x, int y)
{  SetScreenCoord(x, y);
   SpriteRender();
   TRACEDBG("ScrX1=%d  ScrY1=%d  ScrX2=%d  ScrY=%d\n", ScrX1, ScrY1, ScrX2, ScrY2);
   TRACEDBG("cColor=%d 	cSpecular=%d\n", cColor, cSpecular);
   int u1 = TexU1*1000;
   int v1 = TexV1*1000;
   int u2 = TexU2*1000;
   int v2 = TexV2*1000;
   TRACEDBG("TexU1=%d	TexV1=%d	TexU2=%d	TexV2=%d\n", u1, v1, u2, v2);
}
//----------------------------------------------------------------------------//
...
void TD3DSpecialEffect::RenderFilter(bool backup)
{
  ...
  DWORD oldAlphablendState, oldSrcblendState, oldDestblendState;
