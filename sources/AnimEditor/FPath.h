...
bool operator == (const GridCoord& gc1, const GridCoord& gc2)
...
{   //--- Attributs de la Classe ---
  ...
  public:
    ...
    inline int Cout(TMap &m, const GridCoord& gc1, const MapDirection d,
...
{   //--- Attributs de la Classe ---
  ...
  public:
    ...
    // Coût pour avancer d'une case
    // -> doit être supérieur ou égal à la distance entre ces 2 cases
    inline int Cout(TMap &m, const GridCoord& gc1, const MapDirection2 d,
                    const GridCoord& gc2, int hd = -1)
    {  if (hd==-1) hd = HeurDiv;
       // 1) Vérifie si cette case n'est pas occupée par un MUR
