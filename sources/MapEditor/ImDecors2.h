...
// Structure d'un Trapèze (version 0.98)
...
void LoadTileDecors(TileDecors *TileLoading, char *Name, bool fullPath = false);
//----------------------------------------------------------------------------//
...
//  int NumDsList;			// n° dans la liste
    ...
    int  peNumAnim;               // n° spécial de l'animation courante
    ...
    TileDecors *SecondTile;       // Tile représentant l'élément courant
    TileDecors *OldSecondTile;    // ch'tite suavegarde lors du passage d'un TileDecors à un autre
    int  peTileDecorsAssoc;       // n° du TileDecors associé à l'élément courant (provient de 'Element->AnimInfos[anm].numTileDecorsAssoc')
    int  peOldTileDecorsAssoc;    // ancien n° du TileDecors associé à l'élément courant
    ...
    bool DecorsAnime;             // Indique si c'est un objet animé (<=> Element != NULL)

  private:
    void ResultAction();          // Anime les décors
  public:
    void peBouge();
    void Update();                // Gère provisoirement l'IA des décors
    void Fut2PresEtat();          // Gère les transitions de mvts des anims
	...
	void RenderClassikAnims(int ScrX, int ScrY);		// S'occupe d'afficher à l'écran les animations superposées    
};
//----------------------------------------------------------------------------//
...
class DecorsObjetLib
{
  public:
    ...
    void Update();                  // Met à jour les objets contenus ds cette liste

    //--- Nuveau depuis le 13/05/1999 ---
    // Liste des décors ayant subits des modifications se répercutant dans d'autres modules
...
class TLevelTileLib
{
  public:
	...
	void ChangeTileVideoMode();	// Changement de mode-vidéo : restauration et rechargement des surfaces
...
{  int numTileDecorsAssoc;  // n° du TileDecors associé à cette animation
   int dX, dY;              // différences de coordonnées à corriger (soit (dX,dY) pour les anims, soit (dU,dV) pour les TileDecors) 
};
//----------------------------------------------------------------------------//
...
{  public:
        ...
        SpriteFrameList *FrameSuperpList;  // Liste des frames superposées concernant ce décors
        SpriteTileLib *STLFrameSuperp;     // Librairie des sprites du décors

        //--- Infos supplémentaires à AnimSprite aux sujets des animations ---
        TileDecors  *MainTile;             // Tile principal représentant l'objet
        int NoDecorsNumAssoc;              // N° du TileDecors principalement associé à cette anim
   ...
   private:
        TAnimTileDecors();                 // Constructeur par défaut inaccessible
};
//----------------------------------------------------------------------------//
...
// Renvoie le 1er TAnimTileDecors trouvé dans 'ListeObjetsDecors' associé au n° de TileDecors.DecorsNum
// Rmq : sûrement provisoire
TAnimTileDecors* getAnimTileDecors(int numTileDecors);
extern void  ChangeAnimTileDecorsVideoMode(); // Chgt de mode vidéo en plein cours du jeu
