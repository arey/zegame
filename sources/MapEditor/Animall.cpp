...
//---------------------------------------------------------------------- include
#include "ZeNain.h"			// define & disable nécessaires
#ifdef CBUILDER
...
#include "PersoLib.h"
#include "ImDecors2.h"		// Librairie de gestion des décors
#include "consts.h"			// Constantes globales au projet
#include "Animall.h"		// Header du fichier
#ifdef VISUALC
...
AnimSprite::AnimSprite(void)
{
    ...
    Type   =  0;
    NbSprites = 0;
	fAnimSpeed = 1;		// Vitesse par défaut
    FirstSprite = -1;
...
void LoadFileIna(const char* InaNameTresShort,AnimSprite*** pASL, AnimTransition*** pATL,int* pNbAnim,int* pNbTrans,SpriteTilePersoLib **STLPerso, Character* Individu)
{
	...
	#endif // CBUILDER
	// Ouvre le fichier
	if ((f = fopen(InaNameCommon ,"r"))==NULL) 
	{	MessageBoxA(NULL,"FICHIER MANQUANT :", InaNameCommon, 1);
	}
	#else
	TFileVirtual *file = NULL;
	strcpy(InaNameCommon, cPathPersos);
	strcat(InaNameCommon, InaNameTresShort);
	strcat(InaNameCommon, "\\");
	strcat(InaNameCommon, InaNameTresShort);
	strcat(InaNameCommon, ".ina");
	// Ouvre le fichier
	file = fopenFV(InaNameCommon);
	if (file==NULL)	MessageBoxA(NULL,"FICHIER MANQUANT :", InaNameCommon, 1);
	#endif // NEW_FILE_MANAGER
	...
	#else
	while (fgetsFV(BufLine, MaxLine, file) != NULL)  // tant que le fichier n'est pas fini
	#endif  // NEW_FILE_MANAGER
    {
        ...
        switch (Etape)
        {
             ...
             {  // variables utilisées
                ...
                if ((BufLine[0]=='N') && (BufLine[1]==':'))
                  while (BufLine[i]!='\t') texte[c++]=BufLine[i++];
					...
					#else
					MessageBoxA(NULL,"Erreur ds le fichier à la ligne n°"+ComptLine, LPCSTR(InaNameCommon),1);
					#endif
				}
                texte[c]='\0';
                ...
                switch (AS->Type) {
                    ...
                    {   // Crée le tableau dynamique ou se trouveront les n° lus
                        ...
                        for (int j2=0; j2<8; j2++) // choppe les n°images pour chaque orientation
                         	{
                               for (int n=0; n<AS->NbSprites ; n++)
                                 { c=0;
                                  while ((BufLine[i]!=',') && (BufLine[i]!=';') && (BufLine[i]!='\n'))
                                       texte[c++]=BufLine[i++];
                                  texte[c]='\0';
                                  AS->SprRepertoire[j2][n] = atoi(texte);
                                  i++;
                                 };
                            }
                    } break;
            ...
            {   // variables utilisées
                ...
                if ((BufLine[0]=='P') && (BufLine[1]==':'))
                  while (BufLine[i]!='\t') texte[c++]=BufLine[i++];
					...
					#ifdef CBUILDER
					MessageBoxA(NULL,"Erreur ds le fichier à la ligne n°"+ComptLine,InaNameCommon,1);
					#else
					MessageBoxA(NULL,"Erreur ds le fichier à la ligne n°"+ComptLine,LPCSTR(InaNameCommon),1);
					#endif
				}
            ...
            case 3 :  //  #frame
             {  // variables utilisées
				...
				char temp[256];
				strcpy(temp, LPCSTR(BmpSpiName));

                // 2) Vérifie si ce nom existe bien dans le fichier des IDBmpFrame
                if (!IDBmpFrame->IsCle(BmpSpiName))
                {	
					#ifdef CBUILDER
					MessageBoxA(NULL,"Nom de frame *.bmp inexistant à la ligne n° ?",InaNameCommon,1);
					#else
					MessageBoxA(NULL,"Nom de frame *.bmp inexistant à la ligne n° ?",LPCSTR(InaNameCommon),1);
					#endif
				}
				...
				SFL->AddFrame(DS,*STLPerso,filePathBmp.c_str(),filePathSpi.c_str(),numIDFrame);
				#else
				SFL->AddFrame(DS,*STLPerso, LPCSTR(filePathBmp), LPCSTR(filePathSpi), numIDFrame);
				#endif

                // Ajoute le nom contenu dans "texte" à la lib
       // BUG DU 08/07   => decouvert
             ...
             {  // variables utilisées
                ...
                if ((BufLine[0]=='S') && (BufLine[1]==':'))
                  while (BufLine[i]!='\t') texte[c++]=BufLine[i++];
					...
					#ifdef CBUILDER 
					MessageBoxA(NULL,"Erreur ds frameSuperposée à la ligne n°", InaNameCommon, 1);
					#else
					MessageBoxA(NULL,"Erreur ds frameSuperposée à la ligne n°", LPCSTR(InaNameCommon), 1);
					#endif
				}
                texte[c]='\0';
                // l'affecte à AS et traîte qq infos
				...
				{
					#ifdef CBUILDER	
					MessageBoxA(NULL,"Nom de frame inexistant à la ligne n° ?",InaNameCommon,1);
					#else
					MessageBoxA(NULL,"Nom de frame inexistant à la ligne n° ?",LPCSTR(InaNameCommon),1);
					#endif
				}
				...
				{
					#ifdef CBUILDER	
					MessageBoxA(NULL,"Numéro ou nom de frame Existant inexistant à la ligne n° ?", InaNameCommon, 1);
					#else
					MessageBoxA(NULL,"Numéro ou nom de frame Existant inexistant à la ligne n° ?", LPCSTR(InaNameCommon), 1);
					#endif
				}
				...
				{
					#ifdef CBUILDER	
					MessageBoxA(NULL,"Numéro ou nom de type d'Objet inexistant à la ligne n° ?",InaNameCommon,1);
					#else
					MessageBoxA(NULL,"Numéro ou nom de type d'Objet inexistant à la ligne n° ?",LPCSTR(InaNameCommon),1);
					#endif
				}
                ...
                if (typeLoadMemory == "DD") loadBmpInMem = false;
					...
					#ifdef CBUILDER
					MessageBoxA(NULL,"Type de mémoire (MEM ou DD) invalide à la ligne n° ?",InaNameCommon,1);
					#else
					MessageBoxA(NULL,"Type de mémoire (MEM ou DD) invalide à la ligne n° ?", LPCSTR(InaNameCommon),1);
					#endif
				}
                ...
                #ifdef CBUILDER
				Individu->FrameSuperpList->AddFrame(DS, Individu->STLFrameSuperp,filePathBmp.c_str(),filePathSpi.c_str(),numIDFrameSuperposee,loadBmpInMem);
				#else
				Individu->FrameSuperpList->AddFrame(DS, Individu->STLFrameSuperp, LPCSTR(filePathBmp), LPCSTR(filePathSpi), numIDFrameSuperposee,loadBmpInMem);
				#endif
...
{   char InaNameCommon[512];
	...
	#else
	if ((f = fopen(LPCSTR(InaName),"r"))==NULL)
	#endif // CBUILDER
	{
		#ifdef CBUILDER
        MessageBoxA(NULL,"LoadFileInaDecors - FICHIER MANQUANT :",InaName.c_str(),1);
		#else
		MessageBoxA(NULL,"LoadFileInaDecors - FICHIER MANQUANT :", LPCSTR(InaName),1);
		#endif // CBUILDER
	}
	#else	// NEW_FILE_MANAGER
	TFileVirtual *file = NULL;
	strcpy(InaNameCommon, cPathObjets);
	strcat(InaNameCommon, InaNameTresShort);
	strcat(InaNameCommon, ".ina");
	// Ouvre le fichier
	file = fopenFV(InaNameCommon);
	if (file==NULL)	MessageBoxA(NULL,"LoadFileInaDecors - FICHIER MANQUANT :", InaNameCommon, 1);
	#endif	// NEW_FILE_MANAGER
	    ...
	    // Lit les caractères et les ranges ds Entete jusqu'à tabulation
		    ...
		    if ((ASL = (AnimSprite**) malloc(sizeof(AnimSprite*) * NbAnim)) == NULL)
                ...
                case  3 :     //  #frame
                   ...
                   if (NbAnim!=0) (*STLObjets)->fileInaBmpName = new char* [NbAnim];
                   break;
                case 4 :      // #frameSuperposee
        ...
        switch (Etape)
        {
             ...
             {  // variables utilisées
                ...
                switch (AS->Type) {
                    ...
                    {   // Crée le tableau dynamique ou se trouveront les n° lus
                        ...
                        // puis lit les données
                        i+=1;
                        if (!((BufLine[i]=='I') && (BufLine[i+1]==':'))) MessageBoxA(NULL,"Erreur ds le fichier à la ligne n° ?", InaNameCommon,1);
                        i+=2; // pointe sur le prochaine chiffre à lire
                        for (int j2=0; j2<8; j2++) // choppe les n°images pour chaque orientation
                            	{
                               for (int n=0; n<AS->NbSprites ; n++)
            ...
            case 3 :  //  #frame
             {  // variables utilisées
				...
				#ifdef CBUILDER
				SFL->AddFrame(DS,*STLObjets,filePathBmp.c_str(),filePathSpi.c_str(),numIDFrame);
				#else
				SFL->AddFrame(DS,*STLObjets, LPCSTR(filePathBmp), LPCSTR(filePathSpi), numIDFrame);
				#endif
             ...
             {  // variables utilisées
                ...
                if ((BufLine[0]=='S') && (BufLine[1]==':'))
                  while (BufLine[i]!='\t') texte[c++]=BufLine[i++];
                else MessageBoxA(NULL,"Erreur ds frameSuperposée à la ligne n° ?", InaNameCommon, 1);
                texte[c]='\0';
                // l'affecte à AS et traîte qq infos
                AnsiString BmpSpiName = texte;
                AnsiString filePathBmp =  InaNameTresShort;
                ...
                else MessageBoxA(NULL,"Type de mémoire (MEM ou DD) invalide à la ligne n° ?", InaNameCommon ,1);
                ...
                // b) Ajoute la frame bmp à la liste sans le charger mais en chargeant le .spi
				#ifdef CBUILDER
				elem->FrameSuperpList->AddFrame(DS, elem->STLFrameSuperp,filePathBmp.c_str(), filePathSpi.c_str(), numIDFrameSuperposee, loadBmpInMem);
				#else
				elem->FrameSuperpList->AddFrame(DS, elem->STLFrameSuperp, LPCSTR(filePathBmp), LPCSTR(filePathSpi), numIDFrameSuperposee, loadBmpInMem);
				#endif
             ...
             {  // variables utilisées
                ...
                else MessageBoxA(NULL,"ClassikAnim (1): Erreur ds le fichier", InaNameCommon ,1);
                texte[c]='\0';
                ...
                if (!IDAnimation->IsCle(AnimName))
                   MessageBoxA(NULL,"ClassikAnim (2): Erreur ds le fichier", InaNameCommon ,1);
                ...
                else MessageBoxA(NULL,"ClassikAnim (5): Nom de l'anim associée inexistante", InaNameCommon,1);
                texte[c]='\0';
                ...
                {	// 7) Vérifie si cette anim existe bien dans le fichier des IDAnimations
                	AnimName = texte;
                	if (!IDAnimation->IsCle(AnimName))
                   		MessageBoxA(NULL,"ClassikAnim (7): Nom de l'anim associée inexistant ds la lib", InaNameCommon, 1);
