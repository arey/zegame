...
void std::IDMap::LireFileID(AnsiString IDfichier)
{
    FILE *f;
    ...
    // Conserve le nom du fichier lu
    IDFilename = IDfichier;
...
//        MessageBoxA(NULL,"Ligne n°",sattribut.c_str(),0);
		if (sattribut!="")  attribut = sattribut.ToInt();
...
void std::IDMap::AjoutID(AnsiString cle, int attribut)
{
...
void std::IDMap::EcraseID(AnsiString cle, int attribut)
{
    MapID[cle] = attribut;
}
//----------------------------------------------------------------------------//
...
bool std::IDMap::IsCle(AnsiString cle)
{
...
bool std::IDMap::IsElement(int element)
{   // scrute toute la map
    bool trouve = false;
...
bool std::IDMap::IsCouple(AnsiString &cle, int element)
{   Iterat t = MapID.find(cle);
...
int std::IDMap::NumID(AnsiString &cle)
{  Iterat t = MapID.find(cle);
   return (*t).second;
}
//----------------------------------------------------------------------------//
...
void std::IDMap::AjoutClef(AnsiString cle)
{
...
bool std::IDMap::CreateBasedNumID(AnsiString cleDefaut, int attribut)
{
...
void std::IDMap::CreateBasedID(AnsiString cleDefaut)
{
    // Nouvel attribut
    int attrib = MaxAttribut() + 1;
...
void std::IDMap::SauveFileID(AnsiString IDfichier)
{
	FILE *f;

    // Si l'on n'a passé aucun nom de fichier, on prends celui utilisé lors de l'ouverture
    if (IDfichier=="") IDfichier = IDFilename;
...
bool std::IDMap::ChangeClef(AnsiString src, AnsiString dest)
{
...
void std::IDMap::SupprID(AnsiString cle)
{    MapID.erase(cle);
}
//----------------------------------------------------------------------------//
...
AnsiString std::IDMap::CleID(int element)
{   // scrute toute la map
    for (Iterat iter = MapID.begin(); iter!=MapID.end() ; iter++)
    {   int typ = (*iter).second;
