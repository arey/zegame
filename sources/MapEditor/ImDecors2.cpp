...
void LoadTileDecors(TileDecors *TileLoading, char *Name, bool fullPath)
{
    ...
    if (!fullPath)
    {    FileToLoad = CheminEditor + cPathTiles + FileToLoad;
    }
	#ifdef CBUILDER
	strcpy(FileToLoadCommon, FileToLoad.c_str());
	if ((f = fopen(FileToLoadCommon,"rb"))==NULL)
	...
	float DecVersion = 0.0;
	#ifndef NEW_FILE_MANAGER
	fread(&DecVersion, sizeof(float), 1, f);
    if (DecVersion < TileDecVersion)
    { MessageBoxA(NULL,"Version ancienne de sauvegarde", Name, 1);
      fclose(f); return; };
...
{   // Format d'un fichier *.LiD
	// * 15 chars	: nom du fichier
    // * 1 int		: nb de tiles ds cette librairie
    //		Puis pour chaque tile de cette lib
    //		 * 15 chars	: nom du tile
    
    FILE *f;
    char fNomCommon[512];
    #ifdef CBUILDER
	strcpy(fNomCommon, Nom.c_str());
    #else
...
// Fonction provisoire (enfin, je l'espere fortement, qui permet de contrer
// le probleme de l'editeur de banque de tiles ds le MapEditor
AnsiString SearchFileInFolder(char* name, AnsiString path)
{	static const int nbFolders = 7;
	static const AnsiString folders [nbFolders]=
    { "Decors\\dArchi\\", "Decors\\dNatur\\", "Decors\\Mur\\", "Decors\\Objet\\",
      "Sols\\Inter\\",  "Sols\\sArchi\\",  "Sols\\sNatur\\" };
    for (int i=0; i < nbFolders ; i++)
    {	AnsiString test = path + folders[i] + name;
    	FILE *f;
    	if ((f = fopen(test.c_str(),"rb"))!=NULL)
        {   fclose(f);
        	return folders[i];
        }
    }
    ...
    for (int i=0; i < NbTile ; i++)
      {
           ...
           // Recherche le chemin relatif a partir de Tiles\\ dans lequel se trouve ce fichier
           #ifdef CBUILDER
           AnsiString tilePath = SearchFileInFolder(n, CheminApplication + cPathTiles);
           if (tilePath != "")
           {  char n2[512];
              strcpy(n2, tilePath.c_str());
              strcat(n2, n);
              strcpy(n, n2);
           }
           #endif
    	...
    	for (int y=0; y < NbTypeTiles; y++)
           {
              ...
              // Repertoire ds lequel se trouve les .LiD
              AnsiString FileName = CheminApplication;
              FileName += cPathTiles;
...
{	// Format d;un .LiL
	// * 15 chars : nom du niveau
    // * 1 int	: NbTypeTiles
    // * 1 int	: NbTotalTiles
    //		Puis cNbTypeTiles blocs :
    //      * 1 int : NbTilesPerType
    //		* 1 int : TypeTileLib (type de tile : Lib_DECORS ou Lib_SOLS)
    //			Pour chaque tile de ce type
    //				* 15 chars	: nom du tile

    FILE *f;
    char fNom[512];
    #ifdef CBUILDER
	strcpy(fNom, Nom.c_str());
    #else
	strcpy(fNom, Nom);
    #endif
	strcat(fNom, ".LiL");
