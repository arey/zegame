...
{	// Vérifie si l'objet existe bien
    ...
    DelObjectPerIndex(index);
    return true;
}
//----------------------------------------------------------------------------//

//=== MapZone::Supprime l'objet d'après son index ============================//
bool MapZone::DelObjectPerIndex(int objIndex)
{       // Reste plus qu'à le supprimer
	if (iNbJointObject == 1)
    ...
    {	// b) on recopie le derbier à l'endroit de celui que l'on veut supprimer
    	pNumJointObject[objIndex] = pNumJointObject[iNbJointObject-1];
    }
    iNbJointObject--;
    return true;
}
...
{	return ((x >= dDepX) && (x <= x + dWidth) && (y >= dDepY) && (y <= y + dHeight));
}
//----------------------------------------------------------------------------//
