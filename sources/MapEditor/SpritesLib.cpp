...
void MessageBoxZeGame(AnsiString error, char* message)
{
    #ifdef CBUILDER
    MessageBoxA(NULL,error.c_str(), message,1);
    #else
    MessageBoxA(NULL, error, message, 1);
    #endif
}
          ...
          case 3 :
           ...
           {  // variables utilisées
              ...
              {	 AnsiString error = "Nom de l'anim inexistant : ";
				 error+= AnimName;
                 #ifdef CBUILDER
                 MessageBoxA(NULL,error.c_str(), InaName,1);
                 #else
              ...
              {  AnsiString error = "Champs 'NbSprites' inexistant !";
                 MessageBoxZeGame(error, InaName);
              }
