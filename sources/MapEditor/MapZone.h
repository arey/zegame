...
{ // ---------------------------------------------------------- Attributs privés
  ...
  public:
    ...
    bool DelObjectPerIndex(int objIndex);   // Supprime l'objet d'après son index
	...
	//--- Fonctions inline ---
	inline int GetType() { return iType; }			// Renvoi le type de zone
	inline void SetID(int id) { iMapZoneID = id; }	// Attribue un n°ID à la zone
	inline int GetID() { return iMapZoneID; }		// Récupère le n°ID de la zone
